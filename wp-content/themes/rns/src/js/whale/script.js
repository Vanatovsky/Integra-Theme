...
import { createWater, createSky, createMainDecoration, cristalColorAnimateOn, cristalColorAnimationOff, updateSun } from "./main_scene/main_elements"
import { goToLevel1, goToLevel2, goToLevel3, hoverOnUslugaLevel3, hoverOutUslugaLevel3 } from "./levels_animations/levels_animations"
import { createLevel2Texts, raycasterListenerTextesLevel2, openLevel2Page, closeLevel2Page, } from './levels/level2'
...
let composer, mixer
let main_scene_decoration, whale_mesh, whale_mesh_level3, animations, whale_model, whale_gltf, level_2_text_group, text_font
...
const init = () => {
     ...
     * Whale
     */
    createWhale(settings, mixer, animations, scene, level_2_group, whale_gltf, whale_model, whale_mesh, whaleNormalTexture, whaleColorTexture)

    /**
     * Particles
     */
    createParticles(gui, textureLoader, settings, level_2_group, particleTexture, particleTexture_2)
...
fontLoader.load('/wp-content/themes/rns/assets/fonts/intro.json', (font) => { text_font = font })

const particleTexture = textureLoader.load('/wp-content/themes/rns/assets/textures/particles/3.png')
const particleTexture_2 = textureLoader.load('/wp-content/themes/rns/assets/textures/particles/102.png')
...
const tick = () => {
    ...
    if (settings.active_level == 3) {
        ...
        pointCursorGroup.position.y = settings.cursor_level_3_position_y + Math.sin(elapsedTime * 3) / 4
        pointCursorGroup.position.x = settings.cursor_level_3_position_x + Math.cos(elapsedTime * 3) / 4
    ...
    raycasterListenerTextesLevel2(settings, scene, camera, raycaster, pointCursorGroup, level_2_group)
...
document.querySelector('#button_to_level_2').addEventListener('click', () => {
    goToLevel2(settings, camera, scene,
...
document.querySelector('#button_to_level_3').addEventListener('click', () => {
    goToLevel3(settings, camera, scene,
        whale_model, pointCursorGroup, mixer,
        animations)
})
document.querySelector('.listing_uslug_level_3').addEventListener('mouseover', () => {
    hoverOnUslugaLevel3(settings, whale_model, pointCursorGroup)
})
document.querySelector('.listing_uslug_level_3').addEventListener('mouseout', () => {
    hoverOutUslugaLevel3(settings, whale_model, pointCursorGroup)
})


const close_btns = document.querySelectorAll('.pages_level_2 .rf_item .rf_close_item')

for (let cl_btn of close_btns) {
    cl_btn.addEventListener("click", () => { closeLevel2Page(settings, camera, scene, level_2_group) })
}

const close_contacts_buttons = document.querySelector('.rf_whale_level_1_modal_contacts .rf_close_item')
...
const close_services_button = document.querySelector(".listing_uslug_level_3 .rf_close_item")
