jQuery(function($) { 
    ...
    $(document).ready(function() {
        ...
        for (let i = 0; i < tab_boxes.length; i++){
            let area_left_id = $(tab_boxes[i]).children('p').children('.rf_left').attr('id');
            let area_right_id = $(tab_boxes[i]).children('p').children('.rf_right').attr('id');
    ...
    function agregateData (tab_boxes) {
        ...
        for (let i = 0; i < tab_boxes.length; i++){
            ...
            let one_tab_obj = {};
            one_tab_obj.name = $(tab_boxes[i]).children('p').children('.rf_name').val();
    ...
    function createNewTabEl () {
            ...
            const new_el = '<div data-rand="' + rand_num + '" class="rf_admin_tab_box">'+
                ...
                '<p>'+
                    '<textarea id="left' + rand_num + '" class="rf_left" type="text" name="left" style="width:100%;height:100px;float:left;margin-right: 20px;"></textarea><br/><br/>'+
                    '<textarea id="right' + rand_num + '" class="rf_right" type="text" name="right" style="width:100%;height:100px;"></textarea>'+
                '</p>'+
            '</div>'
            return new_el;
    }
    ...
    function createNewOption () {
        return '<div class="rf_one_product_option_box">' +
    ...
    function agregateProductOptions (options_boxes) {
        ...
        for (let i = 0; i < options_boxes.length; i++){
            ...
            one_option_obj.price = $(options_boxes[i]).children('input.rf_option_price').val();
    ...
    function productOptionssInit () {
        ...
        if (options_boxes.length > 0){
            ...
            options_boxes.each(function(){
               $(this).children('.rf_name').on('keyup paste', function(){
                    let all_data = agregateProductOptions(options_boxes);
                    console.log(all_data);
                    $('#rf_product_options_json').val(JSON.stringify(all_data));
               });
               $(this).children('.rf_option_price').on('keyup paste', function(){
                    let all_data = agregateProductOptions(options_boxes);
                    console.log(all_data);
                    $('#rf_product_options_json').val(JSON.stringify(all_data));
                });
