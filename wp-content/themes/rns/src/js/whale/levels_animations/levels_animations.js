...
import {
  ...
  moonLightGoToLevel2,
  moonLightGoToLevel3,
} from "../main_scene/main_lights";
import {
  particlesGoToLevel2,
  particlesGoToLevel3,
} from "../main_scene/particles";
import { setActiveButtonByID, TW } from "./../inc/extra_functions";
import {
  cursor_level_1_material,
  cursor_level_3_meterial,
  whale_material_3,
} from "./../inc/materials";

let animate_position_camera,
  animate_rotation_camera,
  animate_position_cursor,
  animate_rotation_cursor,
  animate_scale_cursor,
  animate_position_whale_model,
  animate_rotation_whale_model,
  animate_level_2_group_rotation,
  animate_scene_fog_far;

/**
 * First animation
 */

export function firstAnimationWhaleAndCamera(
  settings,
  whale_model,
  scene,
  mixer,
  ...
  new TWEEN.Tween(camera.position)
    .to(
      {
        x: settings.camera_position_level_2_x,
        y: settings.camera_position_level_2_y,
        z: settings.camera_position_level_2_z,
      },
      4500
    )
    .start();
  ...
  animate_position_whale_model = new TWEEN.Tween(whale_model.position)
    .to({ x: 0, y: 0, z: 0 }, 6000)
  ...
  animate_rotation_whale_model = new TWEEN.Tween(whale_model.rotation)
    .delay(5000)
    .to({ y: 0.8 }, 5500)
    .easing(TWEEN.Easing.Sinusoidal.Out)
    .start();
  ...
  animate_scene_fog_far = new TWEEN.Tween(scene.fog)
    .delay(2600)
    .to({ far: settings.scene_fog_far_level_2 }, 1000)
...
export function goToLevel1(
  ...
  animate_position_camera = new TWEEN.Tween(camera.position)
    .to(
      {
        x: settings.camera_position_x,
        y: settings.camera_position_y,
        z: settings.camera_position_z,
      },
      1000
    )
    .easing(TWEEN.Easing.Sinusoidal.Out)
...
export function goToLevel2(
  ...
  animate_position_cursor = new TWEEN.Tween(pointCursorGroup.position)
    .to({ x: 0, y: 14, z: 0 }, 1000)
    .start();

  if (animate_scale_cursor !== undefined) {
    animate_scale_cursor.stop();
  }
  animate_scale_cursor = new TWEEN.Tween(pointCursorGroup.scale)
    .to({ x: 1, y: 1, z: 1 }, 1000)
    .start();
...
export function goToLevel3(
  ...
  if (!settings.first_animation_complete) {
    camera.position.set(
      settings.camera_position_level_2_x,
      settings.camera_position_level_2_y,
      settings.camera_position_level_2_z
    );
    firstWhenOpenPage(settings);
  }
  ...
  animate_position_camera = new TWEEN.Tween(camera.position)
    .to({ y: settings.camera_position_level_3_y }, 1000)
    .easing(TWEEN.Easing.Sinusoidal.Out)
    .start()
    .onComplete(() => {
      settings.level_animation_start = false;
  ...
  animate_position_cursor = new TWEEN.Tween(pointCursorGroup.position)
    ...
    .onComplete(() => {
      ...
      animate_position_cursor = new TWEEN.Tween(pointCursorGroup.position)
        .to(
          {
            x: settings.cursor_level_3_position_x,
            y: settings.cursor_level_3_position_y,
            z: -8,
          },
          4000
        )
  ...
  animate_scene_fog_far = new TWEEN.Tween(scene.fog)
    .to({ far: settings.scene_fog_far_level_3 }, 1000)
    .start();
...
function firstWhenOpenPage(settings) {
  ...
  const loader_round_box = document.querySelector(
