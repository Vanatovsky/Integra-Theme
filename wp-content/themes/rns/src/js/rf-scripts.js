...
jQuery(function() {
    ...
    youtube_slider.owlCarousel({
        ...
        navigation: true,
        nav: true,
        navText: ['<img alt="Стрелка влево" src="/wp-content/themes/rns/image/ArrowLeft.svg" />', '<img alt="Стрелка вправо" src="/wp-content/themes/rns/image/ArrowRight.svg" />'],
        dots: false,
    ...
    $('.rf_youtube_slider_previews img').on('click', function() {
        let v = '<iframe width="100%" height="315" src="https://www.youtube.com/embed/' + $(this).data('youid') + '" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
        $('.rf_youtube_video_box').html(v);
    });

    youtube_slider.on('changed.owl.carousel', (event) => {
        // console.log(event.item.index)
        const curentUrl = $('.rf_youtube_slider_previews img').eq(event.item.index).data('youid')
        let v = '<iframe width="100%" height="315" src="https://www.youtube.com/embed/' + curentUrl + '" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
        $('.rf_youtube_video_box').html(v)
    })
    ...
    gallery_previews.children('.owl-stage-outer').children('.owl-stage').children('.owl-item').on('click', function() {
    ...
    horisontal_slider.on('changed.owl.carousel', function(event) {
        $(".rf_horisontal_slider_bottom_navigation .rf_item").removeClass("rf_active")
        $(".rf_horisontal_slider_bottom_navigation .rf_item:nth-of-type(" + (event.item.index + 1) + ")").addClass("rf_active")
    })

    $(".rf_horisontal_slider_bottom_navigation .rf_item").on('click', function(event) {
        $(".rf_horisontal_slider_bottom_navigation .rf_item").removeClass("rf_active")
    ...
    $(".rf-can-open-image").mouseup(function(e) {
        ...
        const move = mouse_x - mouse_x_next > 10 || mouse_x_next - mouse_x > 10 || mouse_y - mouse_y_next > 10 || mouse_y_next - mouse_y > 10;
        if (!move) {
    ...
    $('.checkout_coupon.woocommerce-form-coupon .button[name=apply_coupon]').addClass(['rf-btn', 'rf-btn-primary']);
    ...
    $('.rf_cart_row .rf_item.product-name .rf_open_variations').on('click', function() {
...
function openImageWindow(e) {
    ...
    if (parent_box.hasClass('owl-stage') || parent_box.hasClass('rf_like_owl')) {
        ...
        if (parent_box.hasClass('owl-stage')) {
            ...
            owl_items = $("#" + id_box).children('.owl-stage-outer').children('.owl-stage').children('.owl-item').children('div').children('.rf-item-slider');
        }

        if (parent_box.hasClass('rf_like_owl')) {
            const id_box = parent_box.attr('id');
            owl_items = $("#" + id_box).children('.rf_item').children('.rf_image_ground');
        }

        let arr_string_items = '';
        owl_items.each(function(ind, elem) {
            const src = $(elem).data('full');
            arr_string_items += "<div><div class='rf_owl_imgground' style='background-image:url(" + src + ")' ></div></div>";
        });

        $("#rf_img_show_wrapper").html("<div id='rf_asdf8kjks8' class='owl-carousel owl_show_img'>" + arr_string_items + "</div>");
